...
class MessageClassificationRequestSchema(Dict):
    message: str
...
def classify_message():
    data: MessageClassificationRequestSchema = request.get_json()

    validation_result = check_empty_fields(data, ["message"])
    if not validation_result["result"]:
        return jsonify(validation_result), 400
    ...
    try:
        ...
        # Add node for message classification
        graph.add_node("classify_user_message", main_graph.classify_user_message)

        # Set entry point
        graph.set_entry_point("classify_user_message")

        # Define workflow edges
        graph.add_edge("classify_user_message", main_graph.END)
