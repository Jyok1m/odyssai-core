const endpoints = [
	...
	},
	{
		...
		path: "/api/worlds",
		status: "Operational",
		description: "Get a list of all created worlds",
		response: "Array of all worlds with their details including world_id, world_name, world_description, and metadata",
		exampleResponse: {
			success: true,
			worlds: [
				{
	...
	},
	{
		...
		exampleBody: {
			...
			story_directives:
				"A high-energy blend of slapstick comedy, heartfelt camaraderie, and over-the-top battles. The world is filled with eccentric characters, absurd situations, and frequent meta-humor, balanced by emotional story arcs about friendship, loyalty, and protecting one's home. Magic guild rivalries, alien politics, and ancient mysteries intertwine in unpredictable adventures.",
		},
		exampleResponse: {
			success: true,
			synopsis:
	...
	},
	{
		...
		exampleBody: {
			...
			character_description:
				"a quick-witted swordsman with a knack for trouble, combining samurai discipline with a wizard’s magical prowess. he’s part of a rambunctious guild that takes odd jobs ranging from alien bounties to magical artifact retrieval. his sarcastic humor hides a deep loyalty to his friends.",
		},
		exampleResponse: {
			character_description:
                ...
                ${
									e.method === "GET"
										? `<a href="${e.path}${
												e.queryParams
													? "?" +
													  Object.keys(e.queryParams)
															.map((key) => key + "=" + e.queryParams[key])
															.join("&")
													: ""
										  }" class="try-button" target="_blank">🔗 Test</a>`
										: ""
								}
            </div>
        </div>
